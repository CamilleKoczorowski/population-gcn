...
def create_affinity_graph_from_scores(scores, subject_list, sigma=2.0):
    """
    Construit le graphe de population avec des poids continus (Gaussienne)
    pour les variables quantitatives.
    
    scores       : liste des infos phénotypiques (ex: ['AGE_AT_SCAN', 'SEX', 'SITE_ID'])
    subject_list : liste des IDs des sujets
    sigma        : écart-type pour le noyau gaussien (contrôle la décroissance du poids)
    """
    ...
    for l in scores:
        ...
        # 1. Variables Quantitatives (Âge, QI...) -> Poids Continus
        if l in ['AGE_AT_SCAN', 'FIQ']:
            print(f"Calcul des poids gaussiens pour {l} avec sigma={sigma}...")
            for k in range(num_nodes):
                for j in range(k + 1, num_nodes):
                    try:
                        ...
                        # NOYAU GAUSSIEN : W_ij = exp(-|x_i - x_j|^2 / (2 * sigma^2))
                        # Plus les patients sont proches en âge, plus le poids est proche de 1.
                        weight = np.exp(- (val**2) / (2 * sigma**2))
                        
                        graph[k, j] += weight
                        graph[j, k] += weight
                        
                    except ValueError:  # Donnée manquante
                        pass

        # 2. Variables Catégorielles (Genre, Site...) -> Poids Binaires
        else:
            print(f"Calcul des poids binaires pour {l}...")
            for k in range(num_nodes):
                for j in range(k + 1, num_nodes):
                    try:
