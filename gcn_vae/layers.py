...
class MetricGraphLearner(Layer):
    """
    ...
    def __init__(self, input_dim, embedding_dim=64, act=tf.nn.relu, sparse_inputs=False, **kwargs):
        ...
        with tf.variable_scope(self.name + '_vars'):
            # Poids pour projeter les features vers l'espace latent Z
            self.vars['weights'] = glorot([input_dim, embedding_dim], name='weights_graph')

    def _call(self, inputs):
        if isinstance(inputs, tf.SparseTensor):
            h = dot(inputs, self.vars['weights'], sparse=True)
        else:
            h = dot(inputs, self.vars['weights'], sparse=False)
        
        h = self.act(h) 
        
        # Calcul de la similarité "Metric Learning"
        adj_learned = tf.matmul(h, h, transpose_b=True)
        
        # Activation 
        adj_learned = tf.nn.relu(adj_learned)
        
        # Normalisation (Row-normalize) 
        row_sum = tf.reduce_sum(adj_learned, axis=1, keepdims=True) + 1e-7
        adj_norm = adj_learned / row_sum
