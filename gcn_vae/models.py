...
class DynamicGCN(Model):
    ...
    def _loss(self):
        # Poids decay standard
        for var in self.layers[1].vars.values(): # index 1 car 0 est le learner
            self.loss += FLAGS.weight_decay * tf.nn.l2_loss(var)
        self.loss += masked_softmax_cross_entropy(self.outputs, self.placeholders['labels'],
                                                  self.placeholders['labels_mask'])
    ...
    def _build(self):
        # COUCHE 1 : Graph Learner (Apprend la matrice A)
        # Elle ne change pas les features, elle sort une matrice Adjacence [N, N]
        self.graph_learner = MetricGraphLearner(input_dim=self.input_dim,
                                                embedding_dim=128, # Dimension de l'espace latent
                                                act=tf.nn.tanh,
                                                logging=self.logging)
        
        # Attention : cette couche n'est pas dans self.layers standard car elle ne propage pas X
        # On l'appelle manuellement dans build()
        
        # COUCHE 2 : GCN Layer 1 (Standard)
        self.layers.append(GraphConvolution(input_dim=self.input_dim,
                                            output_dim=FLAGS.hidden1,
                                            placeholders=self.placeholders,
                                            act=tf.nn.relu,
    ...
    def build(self):
        """ Surcharge pour connecter le graphe appris """
        with tf.variable_scope(self.name):
            self._build()

        #Apprendre le graphe dynamique
        ...
        # Diffusion (A * XW)
        layer1_out = tf.matmul(self.adj_dynamic, xw1)
        ...
        layer2_out = tf.matmul(self.adj_dynamic, xw2)
