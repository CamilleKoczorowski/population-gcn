...
def create_affinity_graph_from_scores(scores, subject_list, sigma=2.0):
    """
    Construit le graphe de population avec gestion de sigma et des données manquantes.
    """
...
def get_phenotypic_features(subject_list):
    """
    Récupère un vecteur [Age, Sexe, Site] pour chaque sujet.
    Normalise les valeurs pour qu'elles soient entre 0 et 1 (important pour le neural net).
    """
    import sklearn.preprocessing
    ...
    age_dict = get_subject_score(subject_list, 'AGE_AT_SCAN')
    sex_dict = get_subject_score(subject_list, 'SEX')
    site_dict = get_subject_score(subject_list, 'SITE_ID')
    
    
    unique_sites = list(set(site_dict.values()))
    site_map = {s: i for i, s in enumerate(unique_sites)}

    for sub in subject_list:
        
        try:
            a = float(age_dict[sub])
        except:
            a = 0.0 
        ages.append(a)
        
        
        s = 1.0 if sex_dict[sub] == '1' else 0.0
        sexes.append(s)
        
        
        sites.append(site_map.get(site_dict[sub], 0))

    
    pheno_matrix = np.column_stack((ages, sexes, sites))
    
    
    scaler = sklearn.preprocessing.StandardScaler()
    pheno_matrix = scaler.fit_transform(pheno_matrix)
