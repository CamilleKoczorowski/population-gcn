...
import ABIDEParser_plus_de_pheno as Reader

import train_GCN as Train

def keep_top_k_neighbors(adj_matrix, k=20):
    """ Ne garde que les k plus grandes valeurs pour chaque ligne """
    nrows = adj_matrix.shape[0]
    sparse_adj = np.zeros_like(adj_matrix)
    
    for i in range(nrows):
        # Indices des k meilleures connexions
        top_k_indices = np.argsort(adj_matrix[i])[-k:]
        sparse_adj[i, top_k_indices] = adj_matrix[i, top_k_indices]
    
    # Rendre la matrice symétrique (A doit être égal à A transposée pour les GCN spectraux)
...
def main():
    ...
    graph = Reader.create_affinity_graph_from_scores(['SEX', 'SITE_ID', 'AGE_AT_SCAN'], subject_IDs, sigma=4) #pour age gauss
    ...
    #graph = Reader.create_affinity_graph_from_scores(['SEX', 'SITE_ID', 'AGE_AT_SCAN', 'FIQ'], subject_IDs, sigma=0.000001)
    graph = keep_top_k_neighbors(graph, k=10)
